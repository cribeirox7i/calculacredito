...
function ehTipoValido(valor: string | undefined): valor is TipoFinanciamento {
  return valor === "regulada" || valor === "mercado";
}

function ehIndexadorValido(valor: string | undefined): valor is Indexador {
...
function montarHref(tipo: TipoFinanciamento, indexador: Indexador): string {
  if (tipo === "regulada" && indexador === "tr") return "/financiamento-imobiliario";
  return `/financiamento-imobiliario?tipo=${tipo}&indexador=${indexador}`;
}

type Params = { searchParams: Promise<{ tipo?: string; indexador?: string }> };

async function resolverSelecao(searchParams: Params["searchParams"]) {
  const { tipo: tipoParam, indexador: indexadorParam } = await searchParams;
  const tipo = ehTipoValido(tipoParam) ? tipoParam : "regulada";
  const indexador = ehIndexadorValido(indexadorParam) ? indexadorParam : "tr";
  return { tipo, indexador };
}

export async function generateMetadata({ searchParams }: Params): Promise<Metadata> {
  const { tipo, indexador } = await resolverSelecao(searchParams);
  return {
...
export default async function FinanciamentoImobiliarioPage({ searchParams }: Params) {
  const { tipo, indexador } = await resolverSelecao(searchParams);
      ...
      <div className="mx-auto max-w-3xl space-y-2 px-4 pt-12 sm:px-6">
        <div className="flex flex-wrap gap-2">
          {(["regulada", "mercado"] as TipoFinanciamento[]).map((chave) => (
            <Link
              key={chave}
              href={montarHref(chave, indexador)}
