...
function ehTipoValido(valor: string): valor is TipoConsignado {
  return valor === "inss" || valor === "privado" || valor === "publico";
}

export function generateStaticParams() {
  return (Object.keys(TIPOS) as TipoConsignado[]).map((tipo) => ({ tipo }));
}
...
export default async function ConsignadoPage({ params }: Params) {
  const { tipo } = await params;
