...
import { editarTaxa, removerTaxa } from "./actions-carta-fianca";
import { MensagemAcao } from "./MensagemAcao";
import type { EstadoAcao } from "./tipos-acao";

export function LinhaTaxaCartaFianca({ taxa }: { taxa: TaxaCartaFianca }) {
          ...
          <form action={formAction} className="flex flex-wrap items-end gap-2">
            ...
            <label className="flex flex-col gap-1 text-xs font-medium text-zinc-700 dark:text-zinc-300">
              ...
              <input
                ...
                name="taxaAnual"
                required
                defaultValue={String(taxa.taxaAnual).replace(".", ",")}
            ...
            <label className="flex flex-col gap-1 text-xs font-medium text-zinc-700 dark:text-zinc-300">
              ...
              <input
                ...
                max={120}
                required
                defaultValue={taxa.prazoMaximoMeses}
                className="w-24 rounded-lg border border-zinc-300 px-2 py-1.5 text-sm dark:border-zinc-700 dark:bg-zinc-800"
              />
            </label>
            <label className="flex flex-col gap-1 text-xs font-medium text-zinc-700 dark:text-zinc-300">
  ...
  return (
    <tr className="border-t border-zinc-100 dark:border-zinc-800">
      ...
      <td className="px-4 py-2 text-zinc-900 dark:text-zinc-100">
        {taxa.taxaAnual.toLocaleString("pt-BR", { minimumFractionDigits: 2 })}% a.a.
      </td>
      <td className="px-4 py-2 text-zinc-900 dark:text-zinc-100">{taxa.prazoMaximoMeses} meses</td>
